...
export const run: SlashCommandRunFunction = async (interaction) => {
    try {
        ...
        if (interaction.channelId !== process.env.SUBSCRIBE_CHANNEL_ID) {
            console.log('This command can only be used in the subscribe channel');
            console.log('interaction.channelId:', interaction.channelId);
            console.log('process.env.SUBSCRIBE_CHANNEL_ID:', process.env.SUBSCRIBE_CHANNEL_ID);
            ...
            await interaction.deferReply({ ephemeral: true }); // Defer the reply to buy more time
            ...
            if (!customerId) return void interaction.followUp({
